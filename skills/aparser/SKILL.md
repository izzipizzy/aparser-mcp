...
- **`config_preset="th17"`** — 3 задачи × 17 потоков (+ «Максимум активных заданий»=3),
  сумма ≤ лимиту прокси (~50). Переопредели через env `AP_CONFIG_PRESET` или аргументом.
  Один одиночный bulk без другой очереди — можно `50th` (весь бюджет одной задаче).
...
- **Бюджет потоков = лимит прокси.** Сумма потоков ВСЕХ активных задач ≤ лимиту прокси.
  Варианты: 1 задача × 50 (пресет на 50 потоков), либо 3 × 17 + «Максимум активных заданий=3».
