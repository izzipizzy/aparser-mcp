...
- **`config_preset="default"`** — обычный пресет (есть на любом инсталле A-Parser). На
  сервере включён **«Динамический лимит потоков»** (Настройки → Общие настройки) + «Общий
  лимит потоков» = лимиту прокси (~50), поэтому A-Parser сам держит **сумму** активных
  потоков ≤ лимиту и раздаёт их между заданиями по приоритету (`priority`/`prio` 1–100).
  Бюджет вручную бить (кастомные `50th`/`th17` = 3×17) больше не нужно. Переопредели через
  env `AP_CONFIG_PRESET` или аргументом. (Если динамический лимит выключат — вернись к
  ручному пресету под лимит прокси + «Максимум активных заданий».)
...
- **Бюджет потоков = лимит прокси.** Сумма потоков ВСЕХ активных задач ≤ лимиту прокси.
  ...
  прокси, поэтому хватает обычного пресета (`default`) — A-Parser сам держит сумму ≤ лимиту
  и делит по `priority`. (Без динамического лимита — бей вручную: `th17` = 3×17 + «Максимум
  активных заданий»=3.)
